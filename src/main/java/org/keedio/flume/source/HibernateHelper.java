...
public class HibernateHelper {
	...
	public List<Map<String,Object>> executeQuery() throws InterruptedException {
		...
		try {
			rowsList = query.setFetchSize(sqlSourceHelper.getMaxRows()).setResultTransformer(AliasToEntityMapResultTransformer.INSTANCE).list();
		}catch (Exception e){
		...
		if (!rowsList.isEmpty()){
			if (sqlSourceHelper.isCustomQuerySet()){
					sqlSourceHelper.setCurrentIndex(rowsList.get(rowsList.size()-1).get("INCREMENTAL").toString());
			}
			else
			{
