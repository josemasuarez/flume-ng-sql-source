...
public class SQLSource extends AbstractSource implements Configurable, PollableSource {
	...
	/**
	 * Configure the source, load configuration properties and establish
	...
	public void configure(Context context) {
		...
		bufferedWriter = new BufferedWriter(new ChannelWriter());
	...
	public Status process() throws EventDeliveryException {
		...
		try {
			...
			if (!result.isEmpty()) {
				...
				JsonArray jsonArray = sqlSourceHelper.getAllRowsToJsonArray(result);
				
				for (int i = 0; i < jsonArray.size(); i++) {

					bufferedWriter.write(jsonArray.get(i).toString()+"}");
	...
	private class ChannelWriter extends Writer {
		...
		public void write(char[] cbuf, int off, int len) throws IOException {
			...
			if(sqlSourceHelper.getKeyEvent()!=null){
				headers.put("key",sqlSourceHelper.getKeyEvent());
			}
